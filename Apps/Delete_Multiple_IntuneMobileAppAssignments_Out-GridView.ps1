# Delete Multiple Intune Application assignments for selected Apps
...
$SelectedAzureADGroup = $AllGroups | Select displayName, description, createdDateTime, id, groupTypes | Sort displayName | Out-GridView -Title "Select AzureAD Group to delete from App assignment"  -OutputMode Single
...
# Show Apps in Out-GridView, user can select multiple applications where assignments are deleted
Write-Output "Select Apps in Out-GridView to delete App assignments for group: $($SelectedApp.assignmentTargetGroupDisplayName)"
...
# Delete App assignment for selected Apps
foreach ($SelectedApp in $SelectedApps) {

    # Remove Application Assignment - most important line in this script
    Remove-DeviceAppManagement_MobileApps_Assignments -mobileAppAssignmentId $SelectedApp.assignmentId -mobileAppId $SelectedApp.id
    $Success = $?

    if ($Success) {
        Write-Host "SUCCESS: Deleting $($SelectedApp.assignmentIntent) assignment from application $($SelectedApp.displayname) for group: $($SelectedApp.assignmentTargetGroupDisplayName)" -Foreground Green
    }
    else {
        Write-Host "FAILED: Deleting $($SelectedApp.assignmentIntent) assignment from application $($SelectedApp.displayname) for group: $($SelectedApp.assignmentTargetGroupDisplayName)" -Foreground Red
    }
}
