...
function Change-HTMLTableSyntaxWithRegexpForAppsSortedByDisplayName {
    ...
    $MatchEvaluatorSortedByAppName = {
        ...
        "<tr bgcolor=`"$RowColor`"><td></td><td>$($match.Groups[5].Value)</td><td>$($match.Groups[7].Value)</td><td style=`"font-weight:bold`">$AppDisplayName</td>$($IntentTD)$($match.Groups[15].Value)</td><td></td></tr>"
    }
...
function Change-HTMLTableSyntaxWithRegexpForAppsSortedByAssignmentTargetGroupDisplayName
{
    ...
    $MatchEvaluatorSortedByAssignmentTargetGroupDisplayName = {
        ...
        # This is returned from MatchEvaluator
        "<tr bgcolor=`"$RowColor`"><td></td><td>$($match.Groups[5].Value)</td><td>$($match.Groups[7].Value)</td><td>$AppDisplayName</td>$($IntentTD)<td style=`"font-weight:bold`">$assignmentTargetGroupDisplayName</td>$($match.Groups[18].Value)</td><td></td></tr>"
    }
...
try {
    ...
    if (-not ($UseOfflineCache)) {
        Write-Output "Download AzureAD Group information from Graph API"
        ...
        # Save to local cache
        $AllGroups | ConvertTo-Json -Depth 4 | Out-File "$PSScriptRoot\cache\AllGroups.json" -Force
    } else {
    ...
    if (-not ($UseOfflineCache)) {
        # Get App information from Graph API
        Write-Output "Download Intune App information from Graph API"
        ...
        if ($DownloadAppIcons) {
            ...
            if ($IconFileObject) {
                ...
                try {
                    ...
                    Write-Error "Error converting Base64 to file"
                }
            }
        }
    }
...
'@
    ...
    # Create object with specified attributes and sorting
    $WebAppsByDisplayName = $AppsWithAssignmentInformation | Where-object { ($_.'@odata.type' -eq 'webApp')} | Select-Object -Property c, icon, '@odata.type', displayName, assignmentIntent, assignmentTargetGroupDisplayName, publisher, productVersion, filename, createdDateTime, lastModifiedDateTime, id | Sort displayName
    ...
    $OtherAppsByDisplayName = $AppsWithAssignmentInformation | Where-object {`
        ...
        | Select-Object -Property c, icon, '@odata.type', displayName, assignmentIntent, assignmentTargetGroupDisplayName, publisher, productVersion, filename, createdDateTime, lastModifiedDateTime, id | Sort displayName
    ...
    $htmlObjectAllAppsSortedByAssignmentTargetGroupDisplayName = $AppsWithAssignmentInformation | Select c, icon, '@odata.type', displayName, assignmentIntent, assignmentTargetGroupDisplayName, publisher, productVersion, filename, createdDateTime, lastModifiedDateTime, id | Sort assignmentTargetGroupDisplayName
    ...
    # This is probably quite stupid workaround for problem which might exist :)
    Start-Sleep -Seconds 2

    ############################################################

    # Open html file
    Write-Output "Opening created file: $ReportSavePath\$HTMLFileName"
